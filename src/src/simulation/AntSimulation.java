...
public class AntSimulation {
        ...
        do {
            ...
            choice = scanner.nextLine();  switch (choice) {
                ...
                case "3":
                    ...
                    break;
                case "4":
                    thisSimulation.advanceStage(1); System.out.println("Simulation moved on one stage\n");
                    break;
                case "5":
    ...
    static void displayMenu() {
        ...
        System.out.println("3. Inspect cell");  System.out.println("4. Advance one stage");  System.out.println("5. Advance X stages");  System.out.println("9. Quit");
        System.out.println();
        System.out.print("> ");
    }
    static String getChoice() {
        String choice = scanner.nextLine();  return choice;
    }
    ...
    static class Simulation {
        ...
        protected int numberOfRows,
                numberOfColumns, startingFoodInNest,
                startingNumberOfFoodCells,
                startingNumberOfNests;
        protected int startingAntsInNest, newPheromoneStrength, pheromoneDecay;
        Simulation(ArrayList<Integer>
                           simulationParameters) {
            startingNumberOfNests =
                    simulationParameters.get(0);
            numberOfRows =
                    simulationParameters.get(1);
            numberOfColumns =
                    simulationParameters.get(2);
            startingFoodInNest =
                    simulationParameters.get(3);
            startingNumberOfFoodCells = simulationParameters.get(4);
            startingAntsInNest =
                    simulationParameters.get(5);
            newPheromoneStrength =
                    simulationParameters.get(6);
            pheromoneDecay =
                    simulationParameters.get(7);
            int row, column;
            ...
            for (int count = 2; count <= startingNumberOfNests; count++) {
                ...
                do {
                    allowed = true;
                    row =
                            rGen.nextInt(numberOfRows) + 1;
                    column =
                            rGen.nextInt(numberOfColumns) + 1;
                    for (Nest n : nests)  if (n.getRow() == row && n.getColumn() == column) {
                        allowed = false;  }
                } while (!allowed);
                setUpANestAt(row, column);  }
            for (int count = 1; count <= startingNumberOfFoodCells; count++) {  boolean allowed;
                do {
                    allowed = true;
                    row =
                            rGen.nextInt(numberOfRows) + 1;
        ...
        public void addFoodToCell(int row, int column, int quantity) {
            grid.get(getIndex(row,
                    column)).updateFoodInCell(quantity);
        }
        private int getIndex(int row, int column) {  return (row - 1) * numberOfColumns + column - 1;
        }
        private ArrayList<Integer>
        getIndicesOfNeighbours(int row, int column) {  ArrayList<Integer> listOfNeighbours = new ArrayList<Integer>();
            for (Integer rowDirection : Arrays.asList(-1, 0, 1)) {
                for (Integer columnDirection : Arrays.asList(-1, 0, 1)) {
                    ...
                    if ((rowDirection != 0 || columnDirection != 0) &&
                            (neighbourRow >= 1) &&  (neighbourRow <= numberOfRows) &&
                            (neighbourColumn >= 1) &&
        ...
        private int
        getIndexOfNeighbourWithStrongestPheromone(int row, int column) {
            int strongestPheromone = 0, indexOfStrongestPheromone = -1;
            for (Integer index :
                    getIndicesOfNeighbours(row, column)) {  if (index != -1 &&
                    getStrongestPheromoneInCell(grid.get(index)) > strongestPheromone) {
        ...
        public Nest getNestInCell(Cell c) {  for (Nest n : nests) {
        ...
        public void updateAntsPheromoneInCell(Ant a) {
            for (Pheromone p : pheromones) {  if (p.inSameLocation(a) && p.getBelongsTo() == a.getID()) {

                p.updateStrength(newPheromoneStrength);  return;
            }
            }
            pheromones.add(new
        ...
        public int getStrongestPheromoneInCell(Cell c) {
            ...
            for (Pheromone p : pheromones) {
                if (p.inSameLocation(c)) {  if (p.getStrength() > strongest) {
                    strongest =
                            p.getStrength();
                }
                }
        ...
        public String getAreaDetails(int startRow, int startColumn, int endRow, int endColumn) {  String details = "";
        ...
        public String getCellDetails(int row, int column) {
            ...
            if
            (getNumberOfPheromonesInCell(currentCell) > 0) {  details += "PHEROMONES\n";  for (Pheromone p : pheromones) {  if
            (p.inSameLocation(currentCell)) {
        ...
        public void advanceStage(int
                                         numberOfStages) {
            for (int count = 1; count <= numberOfStages; count++) {
                ArrayList<Pheromone>
                        pheromonesToDelete = new ArrayList<Pheromone>();  for (Pheromone p : pheromones) {  p.advanceStage(nests, ants, pheromones);
                    if (p.getStrength() == 0) {  pheromonesToDelete.add(p);  }
                }
                for (Pheromone p :
                        pheromonesToDelete) {
                    pheromones.remove(p);  }
                for (Ant a : ants) {
                    ...
                    Cell currentCell =
                            grid.get(getIndex(a.getRow(), a.getColumn()));  if ((a.getFoodCarried() > 0) && a.isAtOwnNest()) {

                        addFoodToNest(a.getFoodCarried(), a.getRow(), a.getColumn());

                        a.updateFoodCarried(-a.getFoodCarried());  } else if
                    ((currentCell.getAmountOfFood() > 0) && (a.getFoodCarried() == 0) && (a.getFoodCapacity() > 0)) {
                        int foodObtained; do {
                            foodObtained = rGen.nextInt(a.getFoodCapacity()) + 1;  } while (!(foodObtained <= currentCell.getAmountOfFood()
                                &&
    ...
    static class Cell extends Entity {  protected int amountOfFood;
        ...
        public void updateFoodInCell(int change) {  amountOfFood += change;
        }
    }
    static class Ant extends Entity {
        protected int nestRow, nestColumn, amountOfFoodCarried, stages, foodCapacity;  protected String typeOfAnt;
        static int nextAntID = 1;
        Ant(int startRow, int startColumn, int nestInRow, int nestInColumn) {
            super(startRow, startColumn);  nestRow = nestInRow;
            nestColumn = nestInColumn;  id = nextAntID;
            nextAntID += 1;
            stages = 0;
        ...
        public void updateFoodCarried(int change) {  amountOfFoodCarried += change;  }
        protected void changeCell(int newCellIndicator, IntWrapper rowToChange, IntWrapper columnToChange) {
            if (newCellIndicator > 5) {  rowToChange.value += 1;  } else if (newCellIndicator < 3) {  rowToChange.value -= 1;  }
            if (Arrays.asList(0, 3,
                    6).contains(newCellIndicator)) {
                columnToChange.value -= 1;  } else if (Arrays.asList(2, 5, 8).contains(newCellIndicator)) {
                columnToChange.value += 1;  }
        }
        protected int
        chooseRandomNeighbour(ArrayList<Integer> listOfNeighbours) {
            int rNo = 0;
            do {
                rNo =
                        rGen.nextInt(listOfNeighbours.size());  } while (listOfNeighbours.get(rNo) == -1);
            return rNo;
        }
        public void
        chooseCellToMoveTo(ArrayList<Integer> listOfNeighbours, int
    ...
    static class QueenAnt extends Ant {  QueenAnt(int startRow, int startColumn, int nestInRow, int nestInColumn) {
        super(startRow, startColumn, nestInRow, nestInColumn);
        typeOfAnt = "queen";
    }
    }
    static class WorkerAnt extends Ant {  WorkerAnt(int startRow, int startColumn, int nestInRow, int nestInColumn) {
        super(startRow, startColumn, nestInRow, nestInColumn);
        typeOfAnt = "worker";
        foodCapacity = 30;
    }
        ...
        @Override
        public void
        chooseCellToMoveTo(ArrayList<Integer>
                                   listOfNeighbours, int
                                   indexOfNeighbourWithStrongestPheromone) {  IntWrapper r, c;
            if (amountOfFoodCarried > 0) {  if (row > nestRow) {
                row -= 1;
            } else if (row < nestRow) {  row += 1;
            }
                if (column > nestColumn) {  column -= 1;
                } else if (column < nestColumn) {  column += 1;
                }
            } else if
            (indexOfNeighbourWithStrongestPheromone == -1) {  int indexToUse =
                    chooseRandomNeighbour(listOfNeighbours);  r = new IntWrapper(row);  c = new IntWrapper(column);  changeCell(indexToUse, r, c);  row = r.value;
                column = c.value;
            } else {
                int indexToUse =
                        listOfNeighbours.indexOf(indexOfNeighbourWithStrongestPheromone);
                r = new IntWrapper(row);  c = new IntWrapper(column);  changeCell(indexToUse, r, c);  row = r.value;
                column = c.value;
            }
        }
    }
    static class Nest extends Entity {
        protected int foodLevel, numberOfQueens;  static int nextNestID = 1;
        Nest(int startRow, int startColumn, int startFood) {
            super(startRow, startColumn);  foodLevel = startFood;
            numberOfQueens = 1;
            id = nextNestID;
            nextNestID += 1;
        }
        public void changeFood(int change) {  foodLevel += change;
            if (foodLevel < 0) {
                foodLevel = 0;
            }
        }
        ...
        public void advanceStage(ArrayList<Nest> nests, ArrayList<Ant> ants, ArrayList<Pheromone> pheromones) {
            ...
            if (foodLevel < antsInNestCount) {  antsToCull += 1;
            }
            if (foodLevel < antsInNestCount * 5) {  antsToCull += 1;
                ...
                for (int a = 1; a <= antsToCull; a++) {
                    ...
                    do {
                        rPos =
                                rGen.nextInt(ants.size());
                    } while
                    (!(ants.get(rPos).getNestRow() == row && ants.get(rPos).getNestColumn() == column));  if
                    (ants.get(rPos).getTypeOfAnt().equals("queen")) {  numberOfQueens -= 1;  }
                    ants.remove(rPos);
                }
            } else {
                for (int a = 1; a <=
                        numberOfQueens; a++) {
                    int rNo1 = rGen.nextInt(100);  if (rNo1 < 50) {
                        int rNo2 =
                                rGen.nextInt(100);
                        if (rNo2 < 2) {  ants.add(new QueenAnt(row, column, row, column));
                            numberOfQueens += 1;  } else {
                            ants.add(new WorkerAnt(row, column, row, column));  }
                    }
                }
            }
    ...
    static class Pheromone extends Entity {  protected int strength, pheromoneDecay, belongsTo;
        ...
        @Override
        public void advanceStage(ArrayList<Nest> nests, ArrayList<Ant> ants, ArrayList<Pheromone> pheromones) {
            strength -= pheromoneDecay;  if (strength < 0) {
                strength = 0;
            }
        }
        public void updateStrength(int change) {  strength += change;
        }
